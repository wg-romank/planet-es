...
pub fn tex_unis<'b>(
  ...
  tex: &'b mut Option<UploadedTexture>,
) -> Vec<(&'static str, UniformData<'b>)> {
...
pub fn to_png_texture(ctx: &Ctx, bytes: &[u8]) -> Result<UploadedTexture, String> {
  let texture = image::load_from_memory(bytes).map_err(|e| format!("{:?}", e))?;

  let (width, height) = texture.dimensions();
  let tex = texture.into_rgb8();

  TextureSpec::new(ColorFormat(GL::RGB), [width, height]).upload_u8(&ctx, &tex.as_raw())
}
